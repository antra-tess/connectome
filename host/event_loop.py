        ...
        try:
            while not self._incoming_event_queue.empty():
                ...
                try:
                    ...
                    # Ensure context is detached even if an error occurs
                    if token:
    ...
    async def _process_outgoing_action_queue(self) -> None:
        ...
        try:
            while not self._outgoing_action_queue.empty():
                ...
                # Extract and attach telemetry context
                token = None
                carrier = action_request.pop("telemetry_context", {})
                if carrier:
                    ctx = propagate.extract(carrier)
                    token = context.attach(ctx)
                ...
                try:
                    ...
                    if target_module_name == "ActivityClient":
                        ...
                        if hasattr(self.external_event_router, 'handle_outgoing_action'):
                            ...
                            if hasattr(self.activity_client, 'handle_outgoing_action'):
